...
```sh
git clone <repository-url> ~/sail-runtime
echo 'source ~/sail-runtime/sail-runtime.sh' >> ~/.zshrc
```
...
sr composer install
sr laravel new example-app  # scaffold a new Laravel app in the current directory
sr 8.2 composer install     # combine PHP version and command
```
...
- Xdebug can be configured through `SAIL_XDEBUG_MODE` and `SAIL_XDEBUG_CONFIG`, just like in a regular Sail setup.
